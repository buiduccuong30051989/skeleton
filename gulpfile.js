...
// = Build Style
gulp.task('compile-styles', ['fonts', 'compile-styles-extention'], function(cb) {
  return gulp.src([
    source + '/sass/*.scss',
    // '!'+ source + '/extention/*.scss',
    '!'+ source +'/sass/_*.scss'
  ])
...
gulp.task('compile-styles-extention', function(cb) {
  return gulp.src([
  ...
  .pipe(gulp.dest(dest + '/css/extention'))
  .pipe(browserSync.stream());
});

gulp.task('compile-styles-production', function(cb) {
  return gulp.src([
    source + '/sass/*.scss',
    source + '/sass/extention/*.scss',
    '!'+ source +'/sass/_*.scss'
  ])
  .pipe($.sass(scss.sassOptsProduction)
    .on('error', $.sass.logError))
  .pipe($.postcss(processor))
  .pipe($.rucksack())
  .pipe(header(banner, {pkg: pkg}))
  .pipe(gulp.dest(dest + '/unmincss'));
});
...
gulp.task('compile-js', function(cb) {
  ...
  pump([
    gulp.src(["*.js", "!_*.js"], {cwd: 'source/js/**'}),
    uglify(),
